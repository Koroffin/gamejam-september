    ...
    <>
      {room.floorPositions.map((position, index) => (
        <Fragment key={index}>
          ...
          <Ceiling position={new Vector3(position.x, position.y + 3, position.z)} name={`ceiling-${index}`} />
        </Fragment>
      ))}
      {room.wallsPositions.map(({ position, rotation }, index) => (
        <Fragment key={index}>
          <Wall position={position} rotation={rotation} name={`wall-${index}`} />
        </Fragment>
      ))}

      {room.torchPositions.map(({ position, rotation }, index) => (
        <Torch key={index} position={position} rotation={rotation} name={`torch-${index}`} scale={new Vector3(0.2, 0.2, 0.2)}  />
      ))}
    </>
  );
};
